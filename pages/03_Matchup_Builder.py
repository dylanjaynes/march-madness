...
with st.form("matchup_form"):
    ...
    with col6:
        seed_a = st.number_input(
            "Team A Seed (If NCAAT / Optional)", value=0.0, step=1.0, max_value = 16.0,
            help="Leave 0.0 if not tournament"
        )
    with col7:
        seed_b = st.number_input(
            "Team B Seed (If NCAAT / Optional)", value=0.0, step=1.0, max_value = 16.0,
            help="Leave 0.0 if not tournament"
        )
...
if submitted:
    with st.spinner("Running projection..."):
        # Convert 0 seeds to None so project_game does DB lookup and hybrid routing works
        sa = int(seed_a) if seed_a and seed_a != 0.0 else None
        sb = int(seed_b) if seed_b and seed_b != 0.0 else None
